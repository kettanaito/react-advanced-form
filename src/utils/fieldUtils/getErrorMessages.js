...
export default function getErrorMessages({ validationResult, messages, fieldProps, fields, form }) {
  ...
  const overridesExtras =
    extraKeys && extraKeys.some((extraKey) => defaultResolverKeys.includes(extraKey))

  /* Warn about keys override */
  warning(
    !overridesExtras,
    'Extra keys received from the async response %s overlap with the ' +
      'default resolver arguments %s. Note that the overlapping keys will be overriden in the `%s` field ' +
      'message resolver.',
    JSON.stringify(extraKeys),
    JSON.stringify(defaultResolverKeys),
    fieldProps.get('name'),
  )
  ...
  const resolvedMessages = rejectedRules.reduce((messagesList, rejectedRule, ruleIndex) => {
    ...
    invariant(
      isMessageValid,
      'Expected the error message declaration of the rule `%s` to return a String, ' +
